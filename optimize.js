...
for(let i = 0; i < routeObject.addressList.length; i++)
{
	...
	getCoord(startCoord, routeObject.addressList[i]);
	...
	for(let j = 1; j < routeObject.addressList.length; j++)
	{
		...
		(
			`https://api.mapbox.com/directions/v5/mapbox/driving/${startCoord.latitude.PromiseResult},${startCoord.longitude.PromiseResult};${endCoord.latitude},${endCoord.longitude}?steps=true&geometries=geojson&access_token=${mapboxgl.accessToken}`,
			{ method: 'GET' }
		);
		const json = await query.json();
